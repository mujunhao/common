...
func TestListPlatformUser(t *testing.T) {
	config := consulapi.DefaultConfig()
	config.Address = "192.168.3.6:8500"
	config.Token = ""
	config.Datacenter = "dc1"
	...
	ctx := context.Background()
	p := "1"
	s := v1.InternalUserStatus_USER_STATUS_PENDING
	a := int32(0)
	rsp, err := client.IAM().ListPlatformUser(ctx, 0, 0, &ListPlatformUserOptions{
		P:              &p,
		Status:         &s,
		AssociationNum: &a,
	})
	if err != nil {
		t.Logf("获取用户列表失败（可能服务未启动）: %v", err)
		t.Skip("跳过测试，服务可能未启动")
		return
	}

	t.Logf("成功获取用户列表，总数: %d", len(rsp.Items))
	for i, user := range rsp.Items {
		if i < 3 { // 只打印前3个节点避免输出过多
			t.Logf("用户: Name=%s, Code=%s", user.Nickname, user.UserCode)
		}
	}
}
